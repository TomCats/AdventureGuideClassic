...
function component.Init(components_)
	...
	-- Create the lootContainer
    local lootContainer = CreateFrame("Frame", nil, info)
	lootContainer:SetSize(345, 382)
	lootContainer:SetPoint("BOTTOMRIGHT", -5, 1)

	-- Create the lootScrollBox within the lootContainer
	lootScrollBox = CreateFrame("Frame", nil, lootContainer, "WowScrollBoxList")
	EncounterJournal.encounter.info.LootSrollBox = lootScrollBox
	lootScrollBox:SetSize(345, 382)
	lootScrollBox:SetPoint("BOTTOMRIGHT", -5, 1)

	-- Create the lootScrollBar within the lootContainer
	...
	-- -- Create content for the lootScrollBox
    ...
    local function LootButtonInitalizer(button, lootItem)
		if not button.initialized then
			...
			button.iconBorder:SetPoint("TOPLEFT", button.icon, "TOPLEFT", -1, 1)
			button.iconBorder:SetPoint("BOTTOMRIGHT", button.icon, "BOTTOMRIGHT", 1, -1)

			button.icon = button:CreateTexture(nil, "BACKGROUND")
			button.icon:SetSize(42, 42)
			button.icon:SetPoint("TOPLEFT", 2, -2)
			button.bgTexture = button:CreateTexture(nil, "BORDER")
			button.bgTexture:SetSize(321, 45)
			button.bgTexture:SetPoint("CENTER", button, "CENTER", 0, 0)
			button.bgTexture:SetTexture("Interface\\EncounterJournal\\UI-EncounterJournalTextures")
			button.bgTexture:SetTexCoord(0.00195313, 0.62890625, 0.61816406, 0.66210938)
			button.name = button:CreateFontString(nil, "OVERLAY", "GameFontNormalMed3")
			button.name:SetSize(250, 12)
			button.name:SetPoint("TOPLEFT", button.icon, "TOPRIGHT", 7, -7)
			button.initialized = true
		end

		button.icon:SetTexture(lootItem.iconPath)
...
function component.Show()
	local lootItems = {
		{iconPath = "", name = "Gold Coin"},
		{iconPath = "", name = "Silver Coin"},
		{iconPath = "", name = "Copper Coin"},
		{iconPath = "", name = "Gold Coin"},
		{iconPath = "", name = "Silver Coin"},
		{iconPath = "", name = "Copper Coin"},
		{iconPath = "", name = "Gold Coin"},
